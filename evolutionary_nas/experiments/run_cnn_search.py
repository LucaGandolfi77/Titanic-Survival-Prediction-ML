"""
Run CNN NAS Experiment
======================
Full CNN architecture search across datasets and seeds.
...
from visualization.architecture_diagram import plot_architecture_cnn
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CNN NAS Experiment")
    ...
    if args.dry_run:
        ...
        datasets = [args.dataset] if args.dataset else ["FashionMNIST", "CIFAR10", "CIFAR100"]
    ...
    for dataset_name in datasets:
        ...
        for seed in seeds:
            logger.info(f"=== CNN Search | {dataset_name} | seed={seed} ===")
            set_seed(seed)

            train_loader, test_loader = load_dataset(
                dataset_name, batch_size=64, flatten=False
            )
            ...
            log_path = results_dir / "logs" / f"cnn_{dataset_name}_seed{seed}.json"
            ...
            plot_fitness_evolution(
                result["history"],
                save_path=results_dir / "plots" / f"fitness_cnn_{dataset_name}_s{seed}.png",
                title=f"CNN NAS — {dataset_name} (seed {seed})",
            )
        ...
        best_config = decode(best_genome, "cnn")

        logger.info(f"Full training best CNN for {dataset_name}")
        set_seed(42)
        train_loader, test_loader = load_dataset(
            dataset_name, batch_size=best_config["batch_size"], flatten=False
        )
        model = build_cnn(best_config, info["in_channels"], info["num_classes"])
        ...
        plot_architecture_cnn(
            best_config,
            save_path=results_dir / "plots" / f"arch_cnn_{dataset_name}.png",
        )
        plot_weight_distribution(
            model,
            save_path=results_dir / "plots" / f"weights_cnn_{dataset_name}.png",
        )
        plot_learning_curve(
            train_result.train_losses, train_result.val_losses,
            train_result.train_accs, train_result.val_accs,
            save_path=results_dir / "plots" / f"learning_curve_cnn_{dataset_name}.png",
        )

    out_path = results_dir / "cnn_search_results.json"
