...
G.startDiscovery = function (concertId) {
  ...
  switch (mgType) {
    case 'gossip':
      G.mgGossip(concertId, onComplete)
      break
    case 'cipher':
      G.mgCipher(concertId, onComplete)
      break
    case 'auction':
      G.mgAuction(concertId, onComplete)
      break
    case 'puzzle':
      G.mgPuzzle(concertId, onComplete)
      break
    case 'streetteam':
      G.mgStreetTeam(concertId, onComplete)
      break
    case 'pocketfootball':
      G.mgPocketFootball(concertId, onComplete)
      break
    default:
      G.mgGossip(concertId, onComplete)
      break
...
G.startConcertGame = function (concertId) {
  ...
  // Set up photo mode globals
  G._photoConcertId = concertId
  G._photoCanvas = null

  var onComplete = function (score) {
    // Find canvas for photo mode
    G._photoCanvas = document.getElementById('cg-canvas')
    G.processConcertResult(concertId, score)
  }

  switch (c.artistKey) {
    case 'swift':
      G.cgSwift(concertId, onComplete)
      break
    case 'styles':
      G.cgStyles(concertId, onComplete)
      break
    case 'maneskin':
      G.cgManeskin(concertId, onComplete)
      break
    case 'eilish':
      G.cgEilish(concertId, onComplete)
      break
    case 'beyonce':
      G.cgBeyonce(concertId, onComplete)
      break
    case 'kpop':
      G.cgKpop(concertId, onComplete)
      break
    case 'latin':
      G.cgLatin(concertId, onComplete)
      break
    case 'rock':
      G.cgRock(concertId, onComplete)
      break
    default:
      G.cgBeatDrop(concertId, onComplete)
      break
  }
}
...
G.cgBeatDrop = function (concertId, onComplete) {
  ...
  function startBounce() {
    if (currentRound >= totalRounds || misses >= maxMisses) {
      endGame()
      return
    }
    currentRound++
  ...
  function updateScore() {
    ...
    if (se)
      se.textContent =
        'Round: ' + currentRound + '/' + totalRounds + ' | Hits: ' + hits + ' | Miss: ' + misses + '/' + maxMisses
  }
...
G.processConcertResult = function (concertId, score) {
  ...
  // Apply merch bonuses
  var merchScoreMult = 1.0
  var merchFlatPts = 0
  var merchPtsMult = 1.0
  var merchBudgetBonus = 0
  if (s.merchPurchases) {
    s.merchPurchases.forEach(function (mId) {
      var m = G.MERCH_ITEMS.find(function (item) {
        return item.id === mId
      })
      if (!m) return
      if (m.bonus === 'score') merchScoreMult += m.value
      else if (m.bonus === 'flat') merchFlatPts += m.value
      else if (m.bonus === 'points') merchPtsMult += m.value
      else if (m.bonus === 'budget') merchBudgetBonus += m.value
    })
  }

  // Calculate final points
  var basePointsAfterMerch = Math.round((basePoints + artistBonus + merchFlatPts) * merchScoreMult)
  var totalPoints = Math.round(basePointsAfterMerch * multiplier * hotelMultiplier * merchPtsMult)

  // Budget bonus from merch
  if (merchBudgetBonus > 0 && totalPoints > 0) {
    var mbudgetBonus = Math.round(totalPoints * merchBudgetBonus)
    G.earnBudget(mbudgetBonus)
  }
  ...
  // Screen shake on good scores
  if (score >= 70) {
    G.shake(score >= 90 ? 8 : 4, score >= 90 ? 500 : 300)
  }

  // Crowd noise volume scales with score
  G.setCrowdVolume(score / 100)
