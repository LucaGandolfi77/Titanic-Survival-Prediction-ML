...
G.showBooking = function (concertId) {
  ...
  var routeHtml = G.buildMiniMap(c)
  ...
  // Apply travel upgrade cost multipliers
  var flightTier = G.FLIGHT_TIERS[G.state.travelFlightTier] || G.FLIGHT_TIERS[0]
  var hotelTier = G.HOTEL_TIERS[G.state.travelHotelTier] || G.HOTEL_TIERS[0]
  flights.forEach(function (f) {
    f.price = Math.round(f.price * flightTier.costMult)
  })
  hotels.forEach(function (h) {
    h.price = Math.round(h.price * hotelTier.costMult)
  })
  var flights = [
  ...
  function render() {
    ...
    // Mini-map route visualization
    html += '<div class="booking-minimap">'
    html += '<h3><i data-lucide="map"></i> Route Map</h3>'
    html += G.buildMiniMap(c)
    html += '</div>'
    ...
    if (!canAfford && total > 0)
      html += '<div style="color:var(--danger);font-size:0.8rem;margin-top:4px">Not enough budget!</div>'
    html += '</div>'
  ...
  function confirmBooking(total, travelPts) {
    ...
    // Show confirmation
    el.innerHTML =
      '<div class="fade-in" style="text-align:center;padding:40px 16px">' +
      ...
      '</p>' +
      '<p style="margin:8px 0">' +
      flights[selectedFlight].name +
      ' + ' +
      hotels[selectedHotel].name +
      '</p>' +
      '<p style="color:var(--gold);margin:12px 0">-€' +
      total +
      ' · +' +
      travelPts +
      ' pts</p>' +
