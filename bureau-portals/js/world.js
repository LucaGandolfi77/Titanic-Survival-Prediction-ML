...
export class WorldGenerator {
  constructor(scene) {
    ...
    this.matDoor = new THREE.MeshStandardMaterial({
      map: this.textures.get('woodDark'),
      color: 0x3a3a3a,
      roughness: 0.7,
      metalness: 0.05
    });
    this.matDesk = new THREE.MeshStandardMaterial({
      map: this.textures.get('woodDark'),
      color: 0x8b4513,
      roughness: 0.7,
      metalness: 0.0
    });
    this.matChair = new THREE.MeshStandardMaterial({
    ...
    this.matMetal = new THREE.MeshStandardMaterial({
      map: this.textures.get('metal'),
      color: 0xc0c0c0,
      roughness: 0.3,
      metalness: 0.8
    });
    this.matFiling = new THREE.MeshStandardMaterial({
      map: this.textures.get('woodDark'),
      color: 0x654321,
      roughness: 0.7,
      metalness: 0.0
    ...
    this.matOffice = new THREE.MeshStandardMaterial({
      map: this.textures.get('concrete'),
      color: 0xd4cfc8,
      roughness: 0.8,
      metalness: 0.0
  ...
  _getSharedMaterials() {
    if (this._sharedMaterials) return this._sharedMaterials;
    const tex = this.textures;
    this._sharedMaterials = {
      floor: new THREE.MeshStandardMaterial({
        map: tex.get('concrete'),
        color: 0xffffff,
        roughness: 0.85,
        metalness: 0.05,
        normalMap: tex.get('normal'),
  ...
  buildFloor(room, color = 0x6a6a6a) {
    const mat = this._getSharedMaterials().floor;
    if (mat.color.getHex() !== color) mat.color.setHex(color);
    const floor = new THREE.Mesh(new THREE.BoxGeometry(room.width, 0.1, room.depth), mat);
    floor.position.y = 0;
  ...
  buildCeiling(room, color = 0xe8e4de) {
    const mat = this._getSharedMaterials().ceiling;
    if (mat.color.getHex() !== color) mat.color.setHex(color);
    const ceiling = new THREE.Mesh(new THREE.BoxGeometry(room.width, 0.1, room.depth), mat);
  ...
  buildWalls(room, color = 0x8a8a90) {
    const mat = this._getSharedMaterials().walls;
    if (mat.color.getHex() !== color) mat.color.setHex(color);
    const walls = [
      // Left
      new THREE.Mesh(new THREE.BoxGeometry(0.2, room.height, room.depth), mat),
      // Right
      new THREE.Mesh(new THREE.BoxGeometry(0.2, room.height, room.depth), mat),
      // Front
      new THREE.Mesh(new THREE.BoxGeometry(room.width, room.height, 0.2), mat),
      // Back
      new THREE.Mesh(new THREE.BoxGeometry(room.width, room.height, 0.2), mat)
    ];
  ...
  buildDesk(room, x, y, z) {
    ...
    const top = new THREE.Mesh(new THREE.BoxGeometry(2, 0.08, 1), this.matDesk);
    ...
    for (let i = -1; i <= 1; i += 2) {
      for (let j = -1; j <= 1; j += 2) {
        const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.8), this.matMetal);
  ...
  buildChair(room, x, y, z) {
    ...
    const seat = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.1, 0.5), this.matChair);
    seat.position.y = 0.45;
    group.add(seat);

    const back = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.6, 0.1), this.matChair);
    ...
    for (let i = -1; i <= 1; i += 2) {
      for (let j = -1; j <= 1; j += 2) {
        const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.45), this.matMetal);
  ...
  createFilingCabinet() {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.5), this.matFiling);
  ...
  buildCoffeeMachine(room, x, y, z) {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.8, 0.4), this.matMetal);
    body.castShadow = true;
    group.add(body);

    const spout = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.15), this.matMetal);
  ...
  buildInfiniteCorridor() {
    ...
    const lever = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.5), this.matMetal);
  ...
  buildVoidOffice() {
    ...
    // Desk lamp
    const lampBase = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.3), this.matMetal);
  ...
  buildCopyRoom() {
    ...
    // Copy machines
    const copier = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1.2, 0.6), this.matMetal);
  ...
  buildConferenceRoom() {
    ...
    const table = new THREE.Mesh(new THREE.BoxGeometry(12, 0.08, 1.5), this.matDesk);
