...
G.mgCipher = function (concertId, onComplete) {
  ...
  var letterOnly = letters.filter(function (ch) {
    return ch !== ' '
  })
  // Fisher-Yates shuffle
  for (var i = letterOnly.length - 1; i > 0; i--) {
    var j = Math.floor(Math.random() * (i + 1))
    var tmp = letterOnly[i]
    letterOnly[i] = letterOnly[j]
    letterOnly[j] = tmp
  }
  ...
  function finish(success) {
    ...
    if (success) {
      ...
      el.innerHTML =
        ...
        '<p style="margin:12px 0;color:var(--pink)">"' +
        target +
        '"</p>' +
      ...
      el.innerHTML =
        ...
        '<p style="color:var(--gray-light);margin:12px 0">The answer was: "' +
        target +
        '"</p>' +
