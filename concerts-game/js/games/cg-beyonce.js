...
G.cgBeyonce = function (concertId, onComplete) {
  ...
  function render() {
    ...
    html +=
      '<div id="cg-status" style="font-size:1.1rem;color:var(--gold);margin:10px 0;min-height:28px">Get ready…</div>'
  ...
  function evaluateRound() {
    ...
    if (allCorrect) {
      ...
      G.sfx.success()
      if (statusEl) {
        statusEl.textContent = '✓ Perfect formation!'
        statusEl.style.color = 'var(--success)'
      }
    } else {
      G.sfx.fail()
      if (statusEl) {
        statusEl.textContent = '✗ ' + correct + '/' + pattern.length + ' correct'
