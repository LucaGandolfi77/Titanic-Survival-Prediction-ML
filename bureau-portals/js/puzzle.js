...
export class PuzzleManager {
  ...
  setupObjectives() {
    this.objectives = [
      ...
      {
        id: 'find_calibration',
        text: 'Find Portal Calibration Device (Server Room)',
        completed: false
      },
  ...
  addItemToInventory(item) {
    if (this.inventory.length < this.maxInventory) {
      ...
      if (item.type === 'form' && !this.getObjective('find_form').completed) {
        ...
        item.type === 'key' &&
        !this.getObjective('find_key').completed &&
        !item.data.ismaster
      ) {
        ...
        item.type === 'key' &&
        item.data.ismaster &&
        !this.getObjective('find_master_key').completed
      ) {
  ...
  completeObjective(id) {
    ...
    if (obj) {
      ...
      bus.emit('ui:notify', { message: `OBJECTIVE: ${obj.text}`, type: 'success' });
    }
  }
