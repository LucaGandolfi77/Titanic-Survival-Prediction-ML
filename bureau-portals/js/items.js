...
export class Item {
  ...
  buildStamp() {
    const handle = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x8b4513, roughness: 0.7, metalness: 0.05 })
    );
  ...
  buildCalibration() {
    const box = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x4488ff, roughness: 0.3, metalness: 0.6 })
    );
    this.group.add(box);
...
export class ItemManager {
  ...
  spawnItems() {
    ...
    if (records) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(3, 1.5, -4), 'form', { location: 'cabinet-3' })
      );
    }
    ...
    if (voidOffice) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(1, 1.0, 1), 'key', { location: 'void-desk' })
      );
    }
    ...
    if (voidOffice) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(-1, 1.0, 1), 'stamp', { location: 'void-desk' })
      );
    }
    ...
    if (breakRoom) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(-2, 1.2, 2), 'coffee', {
    ...
    if (conference) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(0, 1.0, 0), 'minutes', {
    ...
    if (director) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(1.5, 1.5, 0), 'key', {
          location: 'director-drawer',
          ismaster: true
        })
      );
    }
    ...
    if (server) {
      this.items.push(
        new Item(this.scene, new THREE.Vector3(-2, 2.0, 0), 'calibration', {
    ...
    if (breakRoom) {
      this.items.push(new Item(this.scene, new THREE.Vector3(2, 1.0, -2), 'battery'));
    }
    if (records) {
      this.items.push(new Item(this.scene, new THREE.Vector3(-3, 0.8, 3), 'battery'));
    }
    if (director) {
      this.items.push(new Item(this.scene, new THREE.Vector3(-2, 1.2, -2), 'battery'));
    }
    ...
    for (const [room, pos] of noteLocations) {
      if (room) {
