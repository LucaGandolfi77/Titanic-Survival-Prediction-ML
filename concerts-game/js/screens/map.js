...
G.renderMap = function () {
  ...
  // Concert dots
  // Flight paths between discovered cities
  var discoveredConcerts = s.discovered
  var discArr = []
  G.CONCERTS.forEach(function (c) {
    if (discoveredConcerts.has(c.id)) discArr.push(c)
  })
  for (var pi = 0; pi < discArr.length; pi++) {
    for (var pj = pi + 1; pj < discArr.length; pj++) {
      var a = discArr[pi]
      var b = discArr[pj]
      var dist = Math.sqrt(Math.pow(a.mx - b.mx, 2) + Math.pow(a.my - b.my, 2))
      if (dist < 500) {
        html +=
          '<line x1="' +
          a.mx +
          ...
          '" y2="' +
          b.my +
          '" stroke="rgba(0,245,212,0.15)" stroke-width="1" stroke-dasharray="6,8" stroke-dashoffset="0">'
        html += '<animate attributeName="stroke-dashoffset" values="0;-14" dur="2s" repeatCount="indefinite"/>'
        html += '</line>'
      }
    }
  }
  G.CONCERTS.forEach(function (c) {
    ...
    if (attended) {
      ...
      dotColor = artist.color
      r = 5
      glowColor = artist.color.replace(')', ',0.3)').replace('rgb', 'rgba')
      opacity = 0.9
      label = artist.emoji
    } else {
      dotColor = '#666'
    ...
    html +=
      ...
      '" cy="' +
      c.my +
      '" r="14" fill="transparent" style="cursor:pointer" class="map-hit" data-cid="' +
      c.id +
      '"/>'
  })
  ...
  if (G.tutorialStep === 2) G.positionSpotlight()

  var svgEl = container.querySelector('svg')

  if (G.FRIENDS && svgEl) {
    G.FRIENDS.forEach(function (f) {
      var svgFriend = document.createElementNS('http://www.w3.org/2000/svg', 'circle')
      svgFriend.setAttribute('cx', f.mx)
      svgFriend.setAttribute('cy', f.my)
      svgFriend.setAttribute('r', '5')
      svgFriend.setAttribute('fill', f.color)
      svgFriend.setAttribute('opacity', '0.6')
      svgFriend.style.cursor = 'pointer'
      var friendHit = document.createElementNS('http://www.w3.org/2000/svg', 'circle')
      friendHit.setAttribute('cx', f.mx)
      friendHit.setAttribute('cy', f.my)
      friendHit.setAttribute('r', '14')
      friendHit.setAttribute('fill', 'transparent')
      friendHit.style.cursor = 'pointer'
      friendHit.addEventListener('mouseenter', function (e) {
        var tt = document.getElementById('map-tooltip')
        tt.innerHTML =
          '<div class="tt-artist" style="color:' +
          f.color +
          '">' +
          f.name +
          ' 👻</div><div class="tt-city">Near ' +
          f.nearCity +
          '</div>'
      ...
      friendHit.addEventListener('mouseleave', function () {
        G.hideMapTooltip()
      })
      friendHit.addEventListener('click', function () {
        G.toast(f.name + ' is chasing concerts near ' + f.nearCity + '! 👻', 2500)
      })
      svgEl.appendChild(svgFriend)
      svgEl.appendChild(friendHit)
    })
  }
...
G.handleMapClick = function (cid, event) {
  ...
  G.toast(
    artist.emoji + ' ' + artist.name + ' in ' + c.city + ' — ' + diff + ' day' + (diff !== 1 ? 's' : '') + ' away! ✈',
    3000
  )
}
